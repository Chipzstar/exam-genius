			...
			try {
				...
				if (type === CHECKOUT_TYPE.PAPER) {
					...
					session = await stripe.checkout.sessions.create({
						...
						mode: 'payment',
						success_url: `${baseUrl}/${PATHS.COURSE}/${input?.course_id}/${input?.unit}/${input?.paper_href}/?subject=${subject}&board=${exam_board}&code=${input?.paper_code}&success=true`,
						cancel_url: `${baseUrl}/${PATHS.COURSE}/${input?.course_id}/${input?.unit}/?subject=${subject}&board=${exam_board}&canceled=true`,
